...
<?php
	$query = "SELECT TV.tableID, TV._number, TV.tableStatus, 
			...
			TV.player1Score, TV.player2Score, TV.bestOf, TV.matchID
	...
	for($i=0; $i < $data_count; $i++)
	{
		...
		$bestOf = $data[$i][9]; $matchID = $data[$i][10];

		$status = castStatus($status);
		tableHeader($status, $number, $matchID);
...
{ ?>
		...
		<?=($b_f=="busy") ? "onclick='openMatchLobby(".$id.");'":""?>>
